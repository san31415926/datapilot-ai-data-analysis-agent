...
> 当前状态：阶段 9 已完成；已接入结构化分析计划、字段与 SQL 校验、一次自动修复和中文页面预览。阶段 10 的工具自动执行与中文报告尚未开始。

## 项目定位

DataPilot 是面向中文电商经营数据的本地自然语言分析工作台。用户上传 CSV 或 XLSX 文件后，可以用中文提问，系统由本地 Ollama 模型生成结构化分析计划，再经过 Pydantic、字段白名单和只读 SQL 校验。目前页面只展示通过校验的计划，不自动执行工具；工具执行和基于真实结果的中文报告属于下一阶段。
